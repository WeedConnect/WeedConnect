...
- [x] **Foro de cannabis** — comunidad / threads / posts / votos. Categorías extendidas (Experiencias, Munchies, Memes, Principiantes). Conectado 100% a la base de datos real de Supabase.
...
- [x] **Subida de fotos** — Supabase Storage. Muro Social (`social-photos`), avatares (`avatars`), hilos del foro (`forum-photos`) y diario de cultivo (`grow-photos`, privado + signed URLs). Todos con bucket + RLS + validación. (Buckets creados e integrados en minúsculas en la base de datos).
...
- [x] **Mapa de spots chill y clubes** — asociaciones legales + miradores + parques + spots para relajarse (PostGIS + tags). Conectado e importando clubes reales desde la base de datos.
- [~] **Noticias** — `info/noticias/page.tsx` con `MOCK_NOTICIAS` and categorías. Falta feed real / RSS.
- [x] **Catálogo de strains** — indica/sativa/híbrido, THC/CBD, efectos. Conectado a la base de datos.
...
1. [x] **Mapa interactivo de asociaciones y spots** — Leaflet + OSM conectado a la base de datos de Supabase (PostGIS).
...
6. [x] **Herramienta de seguimiento de cultivo** — localStorage + Supabase + migración automática. Resumen con stats (días, fase, riego), línea de tiempo visual de fases (Germinación→Curado), botón Finalizar/Reabrir cultivo, exportar JSON, fotos privadas (usuarios con sesión). Conectado 100% a la nube.
...
9. [x] **Gamificación** — sistema de niveles (Semilla→Leyenda) con barra de progreso + grid de 10 logros en el perfil (`src/lib/gamification.ts`, `src/components/gamification/`). Conectado y automatizado con triggers en la base de datos de Supabase.
...
14. [x] **Sistema de votaciones** — VoteButton (up/down) en listado del foro, detalle de hilo y respuestas. Server Action `castVote`/`removeVote` conectado a la base de datos real.
...
- [x] Tema claro/oscuro
- [x] Age gate +18
- [x] Auth (UI + middleware conectado a Supabase real)
...
1. [x] Conectar Supabase real (auth + foro + strains + feed social + votos desde BBDD — ¡Completado!).
2. [x] Subida de fotos (Muro Social, avatar, posts foro y fotos grow log — completado).
3. [~] Mapa con datos reales de clubes y crowd-sourced spots. (Conectado y funcional con datos del seed, listo para expandir).
4. [x] Grow tracker conectado.
5. [~] IA asistente — ya implementada, solo falta que el usuario añada `ANTHROPIC_API_KEY` al `.env.local`.
6. [x] Gamificación (triggers en BBDD para sumar puntos — ¡Completado!).
